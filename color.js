...
var randomSeed = new Math.seedrandom('69');
...
var generateRandomColors = function (number) {
	...
	lastLoadedReduction = Math.floor(randomSeed() * 3), //set a random value to be the first to decrease
	...
	color = function () { //generate a random color
		...
		randVal = function () { //random value between 300 and 510
			return Math.floor(scale(randomSeed()))
		},
		 ...
		 */
		valueToReduce = Math.floor(lastLoadedReduction + 1 + randomSeed() * 2.3) % 3, //which value to reduce
		/*
		 ...
		 */
		valueToIncrease = Math.floor(valueToIncrease + 1 + randomSeed() * 2) % 3, //which value to increase (not the one we reduced)
		increaseBy = randomSeed() + 1; //how much to increase it by
